...
def bounding_callback(msg):
	...
	for idx, box in enumerate(bbox):
		...
		box_ymin = box.ymin
		box_ymax = box.ymax
		box_list.append([box_xmin, box_ymin, box_xmax, box_ymax])
		# blue(1), yellow(0)		
		cone_flag = 1
		...
		'''
		cv2.circle(img, ((box_xmin+box_xmax)/2, box_ymax), 5, (122,0,0), -1)
...
def curve_fitting(arr, sub_num): #arr : sorted yellow_arr or blue_arr [0] : flag / [1] : x  / [2] : y  
    ...
    for i in range(len(arr)-1):
        x_lst = (np.linspace((arr[i][0]), (arr[i+1][0]), sub_num))  
        sub_x = np.concatenate([sub_x, x_lst])
        linear_y = linear_line(arr[i][0], arr[i+1][0], arr[i][1], arr[i+1][1], sub_num)
        sub_y = np.concatenate([sub_y, linear_y])
    
    #popt, pcov = curve_fit(funcx3, sub_x, sub_y)
    #sub_y = funcx3(sub_x, *popt)
    return sub_x, sub_y
...
if __name__ == '__main__':
	...
	while not rospy.is_shutdown(): #cap.isOpened()
		...
		xmax = float(box_xmax)
		ymax = float(box_ymax)
		#box_params = np.float32([xmin, ymin, xmax, ymax])
		#box.append(box_params)
		chk = len(box_list)
		if (len(box_list) > 0):
			for i in range (0, len(box_list)):
				...
				print('*****************')
				if(chk == len(box_list)):
					cv2.rectangle(img, (int(box_list[i][0]), int(box_list[i][1])), (int(box_list[i][2]), int(box_list[i][3])), (0,255,0), 3)
				else :
					print("jedfsjkdnfkjwenfjksdfnkw@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ejfnkwsdjfnwjkefnksjdfnjkwefnskdjfnwejkfnsdkjfnwejkfwkj")
		...
		if (len(box_list) > 0):
			...
			for i in range (0, len(box_list)):
				...
				coneter_x = int((int(box_list[i][0]) + int(box_list[i][2])) / 2)
				#coneter_y = int((int(box_list[i][1]) + int(box_list[i][3])) / 2)
				coneter_y = int(box_list[i][3])
				cone_list.append([coneter_x, coneter_y])
				cone_list = sorted(cone_list, key=lambda x:(x[1],x[0]))                                                                                                                                                  
				#cv2.circle(img, (int(coneter_x), int(coneter_y)), 10, (0,255,0), -1)	
		#x, y = curve_fitting(cone_list, 10)

			for k in range(len(cone_list)-1):
				inter_x = np.array([cone_list[k][0], cone_list[k+1][0]])
				inter_y = np.array([cone_list[k][1], cone_list[k+1][1]])
				f = interpolate.interp1d(inter_x, inter_y)
				slice_x = np.linspace(inter_x[0], inter_x[1], 0)
				f_y = f(slice_x)
				for j in range(len(slice_x)):
					cv2.circle(img, (int(slice_x[j]), int(f_y[j])), 2, (255,0,0), -1)
			cv2.imshow("display", img)
				...
				#x_slice = np.linspace(cone_list[k][0], cone_list[k+1][0], 10)
				
				
		'''
				f_y = linear_line(cone_list[k][0], cone_list[k+1][0], cone_list[k][1], cone_list[k+1][1], 10)
				for j in range(len(x_slice)-1,0, -1):
					cv2.circle(img, (int(x_slice[j]), int(f_y[j])),5, (255,0,0), -1)
		...
		'''
		print("dqwdqwdfasdfqwdasdqwdqwdqwd    ", x, "\n", y)
		for k in range((len(x)-2), 0 , -1): 	
			#cv2.line(img, (int(x[k]),int(y[k])), (int(x[k+1]),int(y[k+1])), (255,255,255), 12)  #curvefitting only yello cone
			#cv2.line(img_transformed_, (int(x[k]),int(y[k])), (int(x[k+1]),int(y[k+1])), (255,0,0), 15)  #curvefitting only yello cone
			cv2.circle(img, (int(x[k]), int(y[k])), 5, (255,0,0), -1)
		cv2.imshow("display", img)
		'''
		...
		# 	out_img, x_location = slidingwindow.slidingwindow(yellow_img)
		# 	if x_location is None:
		# 	    cv2.circle(out_img, (x_location_old, 650), 10, (255, 255, 0), -1)
		# 	    x_location = x_location_old
		# 	else:
		# 	    cv2.circle(out_img, (x_location, 650), 10, (0, 0, 255), -1)
		# 	    x_location_old = int(x_location)
		# else:
		# 	out_img, x_location = slidingwindow.slidingwindow(blue_img)
		# 	if x_location is None:
		...
		# '''
		# alpha = 0.6
		# blended = img_transformed * alpha + out_img * (1-alpha)
		# blended = blended.astype(np.uint8) # 소수점 발생을 제거하기 위함
		# cv2.imshow('img1 * alpha + img2 * (1-alpha)', blended)

		# # ---③ addWeighted() 함수로 알파 블렌딩 적용
		# dst = cv2.addWeighted(img_transformed, alpha, out_img, (1-alpha), 0) 
		# cv2.imshow('cv2.addWeighted', dst)
		# '''
		
		# blended = img_transformed * 1.3 + out_img * (0.8)
		# blended = blended.astype(np.uint8) # 소수점 발생을 제거하기 위함
		# cv2.imshow('img1 * alpha + img2 * (1-alpha)', blended)

		# # ---③ addWeighted() 함수로 알파 블렌딩 적용
		# dst = cv2.addWeighted(img_transformed, 1.3, out_img, 0.8, 0) 
		# cv2.imshow('cv2.addWeighted', dst)
