...
class SlidingWindow:
    ...
    def slidingwindow(self, img):
		...
		for i in range(len(good_line)):
                    cv2.circle(out_img, (nonzero_x[good_line[i]], nonzero_y[good_line[i]]), 1, (255,255,255), -1)
		...
		for window in range(0, window_num):
			...
			#print("@@@@@@@@@@@@@@@@@@@@@@@@@@", x_represent)
		        ...
		        x_represent = np.int(np.polyval(p_left, win_y_high))
                    # 600 700
		    if win_y_low >= 550 and win_y_low < 650:
		        x_location = x_represent + 100 #+150#220#235#230 #245#150 #0.2
