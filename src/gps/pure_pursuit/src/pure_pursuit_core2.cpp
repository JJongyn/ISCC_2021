...
void PurePursuitNode::run(char** argv) {
  ...
  while (ros::ok()) {
    ...
    if (pp_.mode == 20) {
      ...
      // case 1) A,B 인덱스 고정 버전
      // if(p_.mission_flag == 0 && p_.is_delivery_obs_stop_detected){
      //   b_idx_cnt = 0;
      //   p_.mission_flag = 1;
      // }

      // else if(p_.mission_flag == 1 && p_.is_delivery_obs_stop_detected){
      //   b_idx_cnt = 1;
      //   p_.mission_flag = 2;
      // }

      // else if(p_.mission_flag == 2 && p_.is_delivery_obs_stop_detected){
      //   b_idx_cnt = 2;
      //   p_.mission_flag = 3;
      // }

      // if((p_.mission_flag == 1 || p_.mission_flag == 2 || p_.mission_flag == 3) && a_max_index == b_idx_cnt){
